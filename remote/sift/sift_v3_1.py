...
def sift_match(rgb1, rgb2, img1, img2, ocr_reader, T=0.7, rate=0.5, color=(0, 255, 0)):
    ...
    if des1 is not None and des2 is not None:
        ...
        for i, (m, n) in enumerate(matches):
            if m.distance < T * n.distance:
                ki1 = m.queryIdx
                ki2 = m.trainIdx
                if (ki2, ki1) in matchesMap and (kp1[ki1].pt, kp2[ki2].pt) not in coordinate_pairs:
                    matchesMask[i] = [1, 0]
                    count += 1
                    coordinate_pairs.append((kp1[ki1].pt, kp2[ki2].pt))

        #         DrawMatchesFlags_DRAW_RICH_KEYPOINTS
        #         DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS

        c1 = len(set([kp.pt for kp in kp1]))
        c2 = len(set([kp.pt for kp in kp2]))
            ...
            if img_name[-3:] == 'png' or img_name[-3:] == 'jpg':
                ...
                for i in range(0, n - 1):
                    ...
                    for j in range(i + 1, n):
                        ...
                        img3 = sift_match(rgb1, rgb2, img1, img2, ocr_reader, T=T, rate=rate, color=color)
                        if img3 is not None:
...
def detect(detectPath, resPath, T, rate):
    ...
    for journal in tqdm(journalList):
        ...
        for volume in volumeList:
            num1, num2 = IFD(ocr_reader, os.path.join(detectPath, journal, volume),
...
if __name__ == '__main__':
    ...
    res_path = f"/home/hdd1/data/wanfang/{year}/sift_v3_1/{batch}"
