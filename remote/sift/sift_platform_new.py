...
def new_IFD(ocr_reader, to_be_detected_file_path, res_period_path, T, rate, useOCR=True):
    imgs = os.listdir(to_be_detected_file_path)
    n = len(imgs)
    for t in enumerate(imgs):
        print(t)
    for i in range(n - 1):
        for j in range(i + 1, n):
            try:
                img_rgb_i = cv.imread(os.path.join(to_be_detected_file_path, imgs[i]))
                ocr_grey_i = cv.imread(os.path.join(to_be_detected_file_path, imgs[i]), cv.IMREAD_GRAYSCALE)
                ocr_image_i = cv.cvtColor(ocr_grey_i, cv.COLOR_GRAY2RGB)
                img_rgb_j = cv.imread(os.path.join(to_be_detected_file_path, imgs[j]))
                ocr_grey_j= cv.imread(os.path.join(to_be_detected_file_path, imgs[j]), cv.IMREAD_GRAYSCALE)
                ocr_image_j = cv.cvtColor(ocr_grey_j, cv.COLOR_GRAY2RGB)
            except:
                print('读图失败')
                return
            red_color = (0, 0, 255)
            log_file = os.path.join(res_period_path, 'log.txt')
            img_i_j = sift_match(img_rgb_i, img_rgb_j, ocr_grey_i, ocr_grey_j, ocr_image_i, ocr_image_j, ocr_reader, T=T, rate=rate,
                                      color=red_color,
                                      useOCR=useOCR, logfile=log_file, img_path=to_be_detected_file_path)
            if img_i_j is not None:
                cv.imwrite(os.path.join(res_period_path, str(i) + '&' + str(j) + '.jpg'), img_i_j)
            ...
            # if img_i_j is not None:
            #     res_paper_path = res_period_path
...
def detect(detectPath, resPath, T, rate, logger=None, useOCR=True):
    ...
    if useOCR:
        ...
        ocr_reader = None
    new_IFD(ocr_reader, detectPath, resPath, T, rate, useOCR=useOCR)
